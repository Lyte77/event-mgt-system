...
def log_organizer_application_activity(sender, instance, created, **kwargs):
    ...
    collective_name = escape(instance.collective.name)

    if created:
        ActivityLog.objects.create(
            dot_color='var(--amber)',
            html_text=f'<span class="feed-actor">{user_name}</span> submitted an organizer application for <strong>{collective_name}</strong>'
        )
    elif hasattr(instance, 'status'):
        if instance.status == 'APPROVED':
            ActivityLog.objects.create(
                dot_color='var(--green)',
                html_text=f'Application for <strong>{collective_name}</strong> was <span style="color:var(--green);font-weight:600">approved</span>'
            )
        elif instance.status == 'REJECTED':
            reason = escape(getattr(instance, 'rejection_reason', 'Incomplete documents'))
            ActivityLog.objects.create(
                dot_color='var(--red)',
                html_text=f'Application for <strong>{collective_name}</strong> was <span style="color:var(--red);font-weight:600">rejected</span> — {reason}'
            )

@receiver(post_save, sender=Event)
def log_new_event(sender, instance, created, **kwargs):
    if created:
        creator_name =  get_display_name(instance.organizer)
